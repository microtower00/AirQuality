...
MyChart::MyChart (QMap<QString, QtCharts::QAbstractSeries *> serie,
                  MyChart::GraphType gt)
{
...
MyChart::setLineAxis (QMap<QString, QtCharts::QAbstractSeries *> serie)
{
    ...
    if (serie.value ("aqi"))
    ...
    else
        {
            ...
            asseY->setMax (maxValueFromListSeries (serie.values ())
                           + .03 * maxValueFromListSeries (serie.values ()));
            asseY->setTickCount (10);
        }

    QtCharts::QLineSeries *serieLine
        = dynamic_cast<QtCharts::QLineSeries *> (serie.first ());
    int secondiIntervallo
        = (serieLine->points ().at (serieLine->points ().size () - 1).x ()
           - serieLine->points ().at (0).x ())
          / 1000;
    ...
    return QPair<QtCharts::QDateTimeAxis *, QtCharts::QValueAxis *> (asseX,
                                                                     asseY);
}

void
MyChart::buildLineChart (QMap<QString, QtCharts::QAbstractSeries *> serie)
{
    QPair<QtCharts::QDateTimeAxis *, QtCharts::QValueAxis *> assi
        = setLineAxis (serie);
    ...
    setTitle (
        "Andamento della densità dei componenti nel tempo (µg/m³ nel tempo)");
}

void
MyChart::buildAreaChart (QMap<QString, QtCharts::QAbstractSeries *> series)
{
    QPair<QtCharts::QDateTimeAxis *, QtCharts::QValueAxis *> assi
        = setLineAxis (series);
    ...
    for (auto it = series.begin () + 1; it != series.end (); ++it)
        *it = sommaY (dynamic_cast<QtCharts::QLineSeries *> (*it),
                      dynamic_cast<QtCharts::QLineSeries *> (*(it - 1)));

    // per ogni serie nella mappa assegno il nome del componente e i relativi
    // assi
    QtCharts::QAreaSeries *aSeries;
    QtCharts::QLineSeries *lowerSerie;
    for (auto it = series.begin (); it != series.end (); ++it)
        {
            lowerSerie = it != series.begin ()
                             ? dynamic_cast<QtCharts::QLineSeries *> (*(it - 1))
                             : Q_NULLPTR;
            aSeries = new QtCharts::QAreaSeries (
                dynamic_cast<QtCharts::QLineSeries *> (*it), lowerSerie);
    ...
    assi.second->setMax (maxValueFromListSeries (series.values ())
                         + .03 * maxValueFromListSeries (series.values ()));
...
void
MyChart::buildScatterChart (QMap<QString, QtCharts::QAbstractSeries *> serie)
{

    QtCharts::QScatterSeries *sSerie
        = static_cast<QtCharts::QScatterSeries *> (serie.first ());
...
void
MyChart::buildBarChart (QMap<QString, QtCharts::QAbstractSeries *> serie)
{
...
MyChart::sommaY (QtCharts::QLineSeries *upper, QtCharts::QLineSeries *lower)
{
    ...
    for (auto puntoL : puntiLower)
        {
            puntiUpper[puntiLower.indexOf (puntoL)].setY (
                puntiLower.at (puntiLower.indexOf (puntoL)).y ()
                + puntiUpper.at (puntiLower.indexOf (puntoL)).y ());
        }
...
double
MyChart::maxValueFromListSeries (QList<QtCharts::QAbstractSeries *> series)
{
    double max = 0;
    ...
    if (dynamic_cast<QtCharts::QLineSeries *> (series.first ()))
        {
            for (auto serie : series)
                {
                    QtCharts::QLineSeries *lSerie
                        = dynamic_cast<QtCharts::QLineSeries *> (serie);
    ...
    else if (dynamic_cast<QtCharts::QBarSeries *> (series.first ()))
        {
            for (auto serie : series)
                {
                    QtCharts::QBarSeries *barSerie
                        = dynamic_cast<QtCharts::QBarSeries *> (serie);
...
double
MyChart::maxFromSerie (QtCharts::QXYSeries *serie)
{
    double max = 0;
...
double
MyChart::maxFromBarSets (QtCharts::QBarSeries *barre)
{
    double max = 0;
...
MyChart::GraphType
MyChart::stringToEnum (const QString &stringGraf)
{
    if (stringGraf == "Ad aree")
