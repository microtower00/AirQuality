...
DataViewer::DataViewer (Dati *d, QWidget *parent)
    : QMainWindow{ parent }, gbControlliTab (new TableChooser (d)),
...
DataViewer::mostraTable ()
{
    ...
    lbDescr->setText (
        "Visualizzazione dei dati in forma tabellare. È possibile modificare e "
        "salvare i dati aggiornati in un file JSON.");
...
void
DataViewer::setDescr (const MyChart::GraphType &tipoChart)
{
    switch (tipoChart)
        {
        case (MyChart::GraphType::LineG):
            lbDescr->setText (
                "Dopo aver selezionato almeno un componente, premere il tasto "
                "\"Visualizza grafico\". Verrà mostrato un grafico a linee. Il "
                "grafico mostra l'andamento dei componenti scelti (o dell'Air "
                "Quality Index) nel range di tempo selezionato.");
            break;
        case (MyChart::GraphType::BarG):
            lbDescr->setText (
                "Dopo aver selezionato almeno un componente, premere il tasto "
                "\"Visualizza grafico\". Verrà mostrato un istogramma. Il "
                "range di tempo selezionato è stato suddiviso in 10 parti "
                "uguali. Per ogni intervallo viene visualizzata la media dei "
                "valori.");
            break;
        case (MyChart::GraphType::ScatterG):
            lbDescr->setText (
                "Dopo aver selezionato un componente, premere il tasto "
                "\"Visualizza grafico\". Verrà mostrato un plot chart. Viene "
                "visualizzata la distribuzione oraria dei valori per ogni "
                "giorno all'interno del range di tempo selezionato. È "
                "possibile visualizzare dati riguardanti un solo componente "
                "per volta.");
            break;
        case (MyChart::GraphType::AreaG):
            lbDescr->setText (
                "Dopo aver selezionato almeno un componente, premere il tasto "
                "\"Visualizza grafico\". Verrà mostrato un grafico ad aree. "
                "Attraverso questo grafico è possibile confrontare tra loro i "
                "valori dei vari componenti.");
            break;
        case (MyChart::GraphType::RadarG):
            lbDescr->setText (
                "Dopo aver selezionato almeno tre componenti, premere il tasto "
                "\"Visualizza grafico\". Verrà mostrato un grafico radar. Il "
                "grafico è utile per confrontare valori di ogni componente "
                "selezionato nel primo e nell'ultimo giorno del range di tempo "
                "scelto con i valori massimi consentiti (fonte: ARPAV).");
            break;
        default:
            lbDescr->setText ("nessun grafico");
        }
}

void
DataViewer::erroreDialog (const QString &errore)
{
