...
QString StartWindow::fileNonValido
    = "Il file che stai cercando di aprire non è supportato.";
QString StartWindow::cittaNonPresente
    = "La città inserita è vuota o non riconosciuta.";
...
StartWindow::StartWindow (QWidget *parent) : QMainWindow{ parent }
{
    ...
    lbDescr = new QLabel (
        "AirQuality Charts è un programma che consente di creare, importare, "
        "scaricare e modificare dati orari relativi a composizione e qualità "
        "dell'aria di determinate città in determinati intervalli di tempo.");
...
void
StartWindow::chooseFile ()
{
    QString fileName = QFileDialog::getOpenFileName (
        this, "Scegli un file grafico", "", "File JSON (*.json)");
    if (fileName != NULL)
        {
    ...
    try
        {
            ...
            QDate primadataDisp
                = primadataDisp.fromString ("2020-11-27", Qt::ISODate);

            if (inizio >= primadataDisp && fine <= QDate::currentDate ()
                && fine > inizio)
                {
                    lbInfoDati->setText ("Ottengo i dati...");
                    aqr.retrieveHistorical (coords_citta.latitude (),
                                            coords_citta.longitude (), inizio,
                                            fine);
                }
            else
...
Coordinate
StartWindow::coordsResolver (const QString &citta) const
{
    ...
    for (int i = 0; i < json_array.count (); ++i)
        {
            ...
            if (json_obj["city_ascii"] == citta)
                {
                    return Coordinate (json_obj.value ("lat").toDouble (),
                                       json_obj.value ("lng").toDouble ());
                }
        }
...
QJsonDocument &
StartWindow::getCitiesJson () const
{
    QJsonDocument *json = new QJsonDocument (openJson (":/worldcities.json"));

    if (!json->isArray ())
        throw std::invalid_argument (
            "Il file che si sta cercando di aprire non contiene un JSON");

    return *json;
}

QCompleter *
StartWindow::createCompleter () const
{
    ...
    for (int i = 0; i < json_array.count (); ++i)
        listaCitta.append (
            json_array.at (i).toObject ().value ("city_ascii").toString ());
...
void
StartWindow::creoModel (const QJsonDocument *datiDoc,
                        const QDateTime &dataInizio, const Coordinate &c)
{
    lbInfoDati->setText ("");
...
QJsonDocument &
StartWindow::openJson (const QString &path) const
{
    QString val;
    QFile file;
    ...
    QJsonDocument *json
        = new QJsonDocument (QJsonDocument::fromJson (val.toLocal8Bit ()));
    if (json->isNull ())
        throw std::invalid_argument (
            "Il file che si sta cercando di aprire non é valido");
    return *json;
}

void
StartWindow::ottieniDati (const QString &citta, const QDate &inizio,
                          const QDate &fine) const
{
    ...
    if (inizio >= inizioAPI && fine <= QDate::currentDate () && fine > inizio)
        aqr.retrieveHistorical (coords_citta.latitude (),
                                coords_citta.longitude (), inizio, fine);
    else
        throw std::invalid_argument ("Date non valide");
}

void
StartWindow::saveJSonReply (const QJsonDocument *doc) const
{
    if (doc->isObject ())
        {
            QString filename
...
void
StartWindow::apriFinestra (Dati *d)
{
