    ...
    grafico(new MyChartView(data))
{
    ...
    for(auto comp:listaComp) {
        ...
        glComp->addWidget(cbComponenti->buttons().at(listaComp.indexOf(comp)), listaComp.indexOf(comp)/4, listaComp.indexOf(comp)%4);
    }
    ...
    connect(cbGrafici,SIGNAL(currentTextChanged(QString)),this,SLOT(controlliComboBox(QString)));
...
void ChartsChooser::controlliComboBox(const QString& testoCBgrafici)
{
...
void ChartsChooser::attivaComp(const bool& compChecked)
{
